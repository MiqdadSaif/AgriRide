  ...
  Future<void> _submit() async
   {
    ...
    if (_imageFile == null)
    {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: const Text('Please select an image.'), backgroundColor: Colors.red.shade400));
      return;
    }
    if (description.isEmpty) 
    {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: const Text('Description is required.'), backgroundColor: Colors.red.shade400));
      return;
    }
    if (priceText.isEmpty) 
    {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: const Text('Price per hour is required.'), backgroundColor: Colors.red.shade400));
      return;
    }
    final double? pricePerHour = double.tryParse(priceText);
    if (pricePerHour == null || pricePerHour <= 0) 
    {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: const Text('Enter a valid positive price per hour.'), backgroundColor: Colors.red.shade400));
      return;
    }
    ...
    if (pricePerDayText.isNotEmpty) {
      ...
      if (pricePerDay == null || pricePerDay <= 0) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: const Text('Enter a valid positive price per day.'), backgroundColor: Colors.red.shade400));
        return;
      }
    }
    ...
    if (phoneError != null) 
    {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(phoneError), backgroundColor: Colors.red.shade400));
      return;
    }
    ...
    try 
    {
      ...
      final storageRef = FirebaseStorage.instance.ref().child('vehicles').child(user.uid).child('${DateTime.now().millisecondsSinceEpoch}.jpg');
      final uploadTask = await storageRef.putFile(_imageFile!);
      final imageUrl = await uploadTask.ref.getDownloadURL();

      
      final ownerDoc = await FirebaseFirestore.instance.collection('users').doc(user.uid).get();
      ...
      final vehicleData = <String, dynamic>
      {
        ...
        'ownerName': user.displayName ?? ownerData['displayName'],
        'driver': 
        {
          'name': driverName,
          'phone': driverPhone,
      ...
      if (!mounted) return;
      final code = e.code;
      final message = e.message ?? 'Failed to add vehicle.';
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Upload error ($code): $message\nPlease ensure Firebase Storage is configured and App Check is not enforced or properly set up.'),
          backgroundColor: Colors.red.shade400,
        ),
      );
    } catch (_) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: const Text('Unexpected error occurred.'), backgroundColor: Colors.red.shade400));
    } finally {
